...
1,000 items from ChaosNLI's alphaNLI set, judged by the 32-judge panel pinned in
[`../../panel/panel-chaosnli.json`](../../panel/panel-chaosnli.json).

* **Task**: abductive 2-way choice. Labels `1` / `2` (which hypothesis).
* **Arms**: `votes/baseline/` (every judge, every item) and `votes/swap/` (presentation-order
  arm, a 500-item subset re-asked with the options in a different order, `variant` 0/1 — two options, so two orders).
...
* **Counts and hashes**: [`manifest.json`](manifest.json) — rows, unique uids, `parse_fail` cells
  and sha256 per file. Verify with `python3 scripts/verify.py --dataset chaosnli-alphanli-1000`.

**Human label distributions are not redistributed here** (ChaosNLI is CC BY-NC 4.0). Join by
`uid`:

```bash
python3 scripts/join_chaosnli.py --chaosnli /path/to/chaosNLI_v1.0 --dataset alphanli
```

The analysis CLI still uses the v1.0 dataset key `alphanli`.
