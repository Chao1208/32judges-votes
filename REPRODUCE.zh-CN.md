...
4. 固定池渐近线、闭式校准及其与 Monte Carlo 曲线的吻合程度、CC-1000 固定面板、完整的面板
   选择实验及其误差分解，以及剔除占位题后重跑的选择，每项既与保存的未取整值比较，也与论文
   印刷的数字比较。

第 4 步在四个数据集上各枚举全部 C(32,5) = 201,376 与 C(32,7) = 3,365,856 个面板，并在
三个 ChaosNLI 的无占位题集上再枚举一遍，笔记本上约 4.5 分钟；其余步骤只需数秒。成功时终端最后输出 `PASS`，产物写入
...
- `acc` 从不打破并列。每题得分为 `1[gold ∈ M] / |M|`，`M` 为众数标签集合，即均匀打破
  ...
  不同。
- `nu_H_closed_form` 反解上述闭式近似，而非 `reference/calibration_curves.csv` 的 Monte
  Carlo 网格；两者在四个完整 32-judge 面板上相差 0.027%–0.077%。与网格不同，闭式在不足两次
  独立抽样处仍有定义，部分基线正落在那里；这里的值是匹配单位，不是抽样次数。
...
- 规则 A 在 `acc > acc(S0)` 的候选中取 `nu_H` 最大；规则 E 在同一集合中取 `E` 最小；
  规则 D 取 `acc` 最大，再取 `nu_H` 最大。仍并列时取上述顺序中的第一个。准确率按精确整数和比较；`nu_H` 与 `E` 按双精度浮点直接比较，
  不设容差。
- 规则 A 的误差变化可分解为 `E = omega_bar/k + B`，`B = k^-2 sum_{a != b} K_ab` 为带符号的
  交叉项：八个情形中 `omega_bar/k` 都上升，而 `B` 下降得更多。`q_bar` 对归一化内积取平方、
  丢掉了符号，所以它不是进入 `E` 的那个量。
- 剔除任一受占位标签影响的题（MNLI-m 1 题、alphaNLI 5 题）后重跑整个选择，`S0`、A/D/E
  面板与每个枚举计数都不变。
